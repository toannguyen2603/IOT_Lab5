...
router.post("/", async (req, res) => {
  ...
  console.log("Data request:", {...req.body.data});
  ...
  try 
  {
        ...
        // const isChange = Object.entries(value).toString() === Object.entries(valueP).toString() //Find the changing of value
          if(!valuePTerperature || (valuePTerperature.Value-value.Temperature > 0.1 || value.Temperature - valuePTerperature.Value >0.1) || 
          (valuePLight.Value-value.Light > 0.1 || value.Light - valuePLight.Value >0.1) || 
          (valuePHumidity.Value-value.Humidity > 0.1 || value.Humidity - valuePHumidity.Value > 0.1))
          {
